...
export default {
    ...
    activate(state) {
        ...
        this.outputPanel.element.classList.add('native-key-bindings');
        this.outputPanel.element.setAttribute('tabindex', -1);
    ...
    execute(filepath, functionName) {
        ...
        if (atom.config.get('atom-phpunit.saveBeforeTest') &&
         atom.workspace.getActiveTextEditor().isModified()) {
             return atom.workspace.getActiveTextEditor().save().then(() => {
                 return this.execute(filepath, functionName);
             });
         }
        ...
        if (fs.existsSync(path.join(this.getProjectFolderPath(), 'phpunit.xml'))) {
            cmd += ` --configuration=${this.getProjectFolderPath()}/phpunit.xml`;
        }
